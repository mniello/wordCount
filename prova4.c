...
void wordCount(int *countParola, char** parola, int* size,int inizioByte,int fineByte,int indiceInizioFile,fileS* fileDaLeggere,int myrank); // FUNZIONE PER CONTARE LE PAROLE 
...
int main(int argc, char**argv) {
	...
	wordCount(countParola,parola,&size,inizioByte,fineByte,indiceInizioFile,fileDaLeggere,myrank);
...
void wordCount(int *countParola, char** parola, int* size,int inizioByte,int fineByte,int indiceInizioFile,fileS* fileDaLeggere, int myrank) {
/*	INPUT:
	...
	while(byteLetti<byteDaLeggere) {		//PRIMO CICLO DI CONTROLLO OGNI PROCESSORE LEGGE IN TOTALE UN NUMERO DI BYTE <= DI ByteDaLeggere
		...
		if(file!=NULL) {
			...
			while(byte<byteDaLeggere) {	//SECONDO CICLO DI CONTROLLO UTILE SICCOME UN PROCESSO POTREBBE DOVER LEGGERE DA PIU' FILE
				...
				//CONTROLLO SE IL PROCESSO DEVE SPOSTARSI INDIETRO SICCOME IL PROCESSO P-1 HA SALTATO LA PAROLA SICCOME SI TROVAVA ALLA FINE DEI SUOI BYTE
